...
These handlers support the `Http` classes and plain `Protocol` objects for responses.
You can customize the _response body_ format using a formatter function.
...
The default formatter creates a `{ status, data, meta }` object, mapping HTTP status codes:
...
You can provide a custom formatter when creating `DefaultHandlers`:
...
- Otherwise, it sends a 500 Internal Server Error response with the default message.
...
Handles schema validation errors (using Zod). It always throws a `Http.BadRequest` error with a default message.
...
Called _after_ a response has been sent (headers already sent). Primarily used for logging or cleanup. The default implementation logs the response data.

```typescript
onPostResponse(data: unknown, conn: Conn): Promise<void> | void {
    console.log(`Suvidha(onPostResponse): ${util.inspect(data, { depth: 10 })}`);
}
```
