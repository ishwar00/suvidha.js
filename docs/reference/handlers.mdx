...
Before we start, let's understand the flow of request and response handling.

```ascii Flow [expandable] {7, 37, 39, 50, 52}
[Start Request]
      |
      v
...
All the above highlighted steps are handled by the **handlers**.
Here is the code version of the above flow(and most of the `handler` method's code and library's core logic).


```ts core [expandable] {4, 19, 23, 31, 34, 40}
const conn = { req, res };
try {
    // parse() calls onSchemaErr() handler
...
<Warning>
**Never throw** - this is the last error handler
</Warning>
...
#### onDualResponseDetected (Conflict Handler)
...
if (headersSent) {
  if (handlerOutput !== undefined) {
    onDualResponseDetected(handlerOutput);
  }
  return;
}
```
 ...
 * @param next - express' next function
 */
onDualResponseDetected(outputOrErr: unknown, conn: Conn, next: NextFunction): Promise<void> | void;
```
...
class CustomHandlers implements Handlers {
    ...
    async onDualResponseDetected(output: unknown, { req }: Conn) {
