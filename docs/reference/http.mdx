...
Suvidha's `Http` classes simplify responses. Default handlers support them _and_ plain `Protocol` objects. If you write your own handlers, you need to support these if you want to use them.
 ...
 * @property {Headers} [headers] - Optional response headers.  A record where keys are header names and values are header values.
...
This is the foundation for all the other Http classes.
...
```typescript [expandable]
/**
 * Represents a 400 Bad Request response.  Has a default body message.
 */
...
other class follow the same pattern.

### Error Handling with `Http` Classes

You can throw instances of the appropriate `Http` error class.
...
Return plain `Protocol` objects if preferred:
...
<Warning>
    {" "}
    Don't throw plain `Protocol` objects, they are not processed by
    `DefaultHandlers`'s `onErr` handler. Use `Http` classes instead.{" "}
</Warning>
