...
interface to define handlers for `onSchemaErr`, `onComplete`, `onDualResponseDetected` and `onErr`.
...
To use Suvidha solely for data validation with TypeScript inference, use the `DefaultHandlers` class and customize error responses as needed.

```ts {27}
// Customize the error response
class CustomHandlers extends DefaultHandlers {
    override onSchemaErr(_: ZodError, conn: Conn) {
        const fmt = {
            success: false,
