...
Suvidha's `CtxRequest` extends the standard `Request` object with a `context` property.  
Existing handlers that accept `Request` will also work with `CtxRequest`. Access the context using `CtxRequest`.
...
function validateBody(body: any, schema: z.ZodTypeAny) {
...
app.post(
    ...
    authenticate,
    (req, res, next) => {
        if (!validateBody(req.body, BookSchema)) {
...
const authenticate = async ({ req }: Conn) => {
